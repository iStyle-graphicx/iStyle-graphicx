...
export function CustomerDashboard({ user, onRequestDelivery, onNavigate }: CustomerDashboardProps) {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      // Fetch deliveries with driver info
      const { data: deliveries } = await supabase
        .from("deliveries")
        .select(`
          *,
          drivers!deliveries_driver_id_fkey(id, rating, vehicle_type)
        `)
      ...
      // Set active deliveries with driver info
        ...
        .map((delivery) => ({
          ...
          driver: delivery.driver_id
            ? {
                ...
                rating: delivery.drivers?.rating || 0,
                vehicle_type: delivery.drivers?.vehicle_type || "Unknown",
              }
            : undefined,
        }))
      ...
      const { data: favorites } = await supabase.from("favorite_addresses").select("*").eq("user_id", user.id).limit(3)
        ...
        <Card className="bg-white/10 backdrop-blur-md border-white/20">
          <CardContent className="p-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-2xl font-bold text-white">{stats.averageRating || "N/A"}</p>
            ...
            {favoriteAddresses.map((address) => (
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex-1">
                    <p className="text-sm text-white font-medium">{address.label}</p>
